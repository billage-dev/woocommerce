...
$myUpdateChecker=Puc_v4_Factory::buildUpdateChecker(
  'https://github.com/billage-dev/woocommerce',
  __FILE__,
  'billage'
);
$myUpdateChecker->setAuthentication($token);
//$token is created in plugin-update-checker.php
//Mirar link --> authentication required in github --> https://github.com/YahnisElsts/plugin-update-checker/issues/479
$myUpdateChecker->setBranch('main');

ini_set('precision', 15);

$plugins_url = plugins_url();
if(!isset($requestCalled)){
  $requestCalled=false;
}
class Billage
{
  ...
  function writeInvoice($order_id) {
    //La variable requestCalled está creada per evitar factures duplicades (2 peticions al meteix arxiu)
    global $requestCalled;
    if( (!isset($_COOKIE['order']) || $_COOKIE['order']!=$order_id) && $requestCalled == false){
...
add_action( 'woocommerce_order_status_completed','writeInvoice');
